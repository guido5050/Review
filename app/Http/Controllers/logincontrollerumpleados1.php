...
class logincontrollerumpleados extends Controller {
	...
	public function show() {
        ...
        if ( auth::guard('empleados')->guest() ) {
            ...
            return redirect()->route('turnos_admin');
        }
    }
    ...
    public function register(request $data) {
        ...
        if (is_null($data['usuario']) || $data['usuario'] == '') {
            ...
            $usuario = \App\usuarios_empleado::create($data->all());
            ...
            $data['contrasena'] = bcrypt($data['contrasena']);
            $usuario = \App\usuarios_empleado::create($data->all());
            $usuario = \App\usuarios_cliente::create($data->all());
            Auth::guard('empleados')->logout(); 
    ...
    /*public function update_user(request $data) {
        $id = auth()->guard('empleados')->user()->id_empleados;
        $rules = [
            'nombre_completo' => ['required', 'string', 'max:255'],
            'email' => ['required', 'string', 'email', 'max:255', 'unique:usuarios_empleados,email,'.$id . ',id_empleados'],
            'usuario' => ['unique:usuarios_empleados,usuario,'.$id . ',id_empleados'],
            'num_identificacion' => ['required','max:50'],
            'num_telefono' => ['required'],
            'cargo' => ['required','max:2'],
        ];
    ...
    public function aut_user(request $data) {
		...
		//dd($xyz);
        if ($xyz) {
            $usuario_estado_valid = \App\usuarios_empleado::where('usuario','=',$data['username'])->pluck('activo')->first();
            ...
            if ($usuario_estado_valid == 0 || is_null($usuario_estado_valid) || $usuario_estado_valid == ' ' || $usuario_estado_valid != 1) {
                ...
                $usuario_empleado = \App\usuarios_empleado::where('id_empleados','=',$id_empleados)->first();
    ...
    public function update(request $data) {
        $empleado = \App\usuarios_empleado::where('id_empleados','=',$data->get('id_empleados'))->first();
        $rules = [
            'nombre_completo' => ['required', 'string', 'max:255'],
        ...
        $user_update = \App\usuarios_empleado::where('id_empleados','=',$data['id_empleados'])->update($data->except('_token')); 
        if ($user_update) {
